...
function initIdContact(typeAuth) {
  if (typeof PortPub == "undefined" || PortPub.length == 0)
    PortPub = "https://www.impots.gouv.fr";
  //if((typeof(PathCFP)=='undefined') || PathCFP.length==0)PathCFP="/portail/contacts";
  if (typeof PathCFP == "undefined" || PathCFP.length == 0)
    PathCFP = "/contacts";
  cfp["default"] =
    '<a class="CFP" target="_blank" rel="noopener" title="Nouvelle fen&ecirc;tre" href="' +
    PortPub +
    PathCFP +
    "NOID" +
    '">centre des Finances publiques <span class="dgfipicon dgfipicon-sortie-page" title="Nouvelle fen&ecirc;tre">.</a>';
  cfp["AMALGAME"] =
    '<a class="CFP" target="_blank" rel="noopener" title="Nouvelle fen&ecirc;tre" href="' +
    PortPub +
    PathCFP +
  ...
  for (cas in idContact) {
    if (cfp[cas] !== undefined) {
      if (cas == "BLOCAGE") {
        ...
        cfp[cas] = cfp[cas].replace(
          "NOID",
          idContact[cas] ? "?" + idContact[cas] : idContact[cas]
        );
    } else {
      cfp[cas] = cfp["default"].replace(
        "NOID",
        idContact[cas] ? "?" + idContact[cas] : idContact[cas]
      );
    }
